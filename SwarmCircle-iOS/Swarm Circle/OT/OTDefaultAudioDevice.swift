...
class OTDefaultAudioDevice: NSObject {
    ...
    func setAudioBus(_ audioBus: OTAudioBus?) -> Bool {
        ...
        audioFormat.sampleRate = OTDefaultAudioDevice.kSampleRate
    ...
    fileprivate var recordingVoiceUnit: AudioUnit? = nil
    fileprivate var playoutVoiceUnit: AudioUnit? = nil
    ...
    fileprivate func setupAudioUnit(withPlayout playout: Bool) -> Bool {
        ...
        streamFormat.mSampleRate = Float64(OTDefaultAudioDevice.kSampleRate)
        ...
        if playout {
            AudioUnitSetProperty(playoutVoiceUnit!, kAudioOutputUnitProperty_EnableIO,
                                 kAudioUnitScope_Output, OTDefaultAudioDevice.kOutputBus, &value,
                                 UInt32(MemoryLayout<UInt32>.size))
            
            AudioUnitSetProperty(playoutVoiceUnit!, kAudioUnitProperty_StreamFormat,
                                 kAudioUnitScope_Input, OTDefaultAudioDevice.kOutputBus, &streamFormat,
            ...
            AudioUnitSetProperty(playoutVoiceUnit!, kAudioOutputUnitProperty_EnableIO, kAudioUnitScope_Input,
                                 OTDefaultAudioDevice.kInputBus, &enableInput, UInt32(MemoryLayout<UInt32>.size))
        } else {
            AudioUnitSetProperty(recordingVoiceUnit!, kAudioOutputUnitProperty_EnableIO,
                                 kAudioUnitScope_Input, OTDefaultAudioDevice.kInputBus, &value,
                                 UInt32(MemoryLayout<UInt32>.size))
            AudioUnitSetProperty(recordingVoiceUnit!, kAudioUnitProperty_StreamFormat,
                                 kAudioUnitScope_Output, OTDefaultAudioDevice.kInputBus, &streamFormat,
                                 UInt32(MemoryLayout<AudioStreamBasicDescription>.size))
            // Disable Output on record
            var enableOutput = 0
            AudioUnitSetProperty(recordingVoiceUnit!, kAudioOutputUnitProperty_EnableIO, kAudioUnitScope_Output,
                                 OTDefaultAudioDevice.kOutputBus, &enableOutput, UInt32(MemoryLayout<UInt32>.size))
        }
    ...
    fileprivate func setupPlayoutCallback() {
        ...
        var renderCallback = AURenderCallbackStruct(inputProc: otRenderCb, inputProcRefCon: selfPointer)
        AudioUnitSetProperty(playoutVoiceUnit!,
                             kAudioUnitProperty_SetRenderCallback,
                             kAudioUnitScope_Input,
                             OTDefaultAudioDevice.kOutputBus,
                             &renderCallback,
    ...
    fileprivate func setupRecordingCallback() {
        let selfPointer = Unmanaged.passUnretained(self).toOpaque()
        var inputCallback = AURenderCallbackStruct(inputProc: otRecordCb, inputProcRefCon: selfPointer)
        AudioUnitSetProperty(recordingVoiceUnit!,
                             kAudioOutputUnitProperty_SetInputCallback,
                             kAudioUnitScope_Global,
                             OTDefaultAudioDevice.kInputBus,
                             &inputCallback,
                             UInt32(MemoryLayout<AURenderCallbackStruct>.size))
...
extension OTDefaultAudioDevice: OTAudioDevice {
    ...
    func estimatedRenderDelay() -> UInt16 {
        return UInt16(min(self.playoutDelay, UInt32(OTDefaultAudioDevice.kMaxPlayoutDelay)))
    }
    ...
    func stopRendering() -> Bool {
        ...
        guard let playoutVoiceUnit = self.playoutVoiceUnit else {return true}
    ...
    func stopCapture() -> Bool {
        ...
        guard let recordingVoiceUnit = self.recordingVoiceUnit else { return true}
...
// MARK: - AVAudioSession
extension OTDefaultAudioDevice {
    ...
    fileprivate func handleInterruptionEvent(type: Int?) {
        ...
        case AVAudioSession.InterruptionType.ended.rawValue:
            configureAudioSessionWithDesiredAudioRoute(desiredAudioRoute: OTDefaultAudioDevice.kAudioDeviceBluetooth)
...
//
//        notificationCenter.addObserver(self, selector: #selector(OTDefaultAudioDevice.onInterruptionEvent),
//                                       name: AVAudioSession.interruptionNotification, object: nil)
//        notificationCenter.addObserver(self, selector: #selector(OTDefaultAudioDevice.onRouteChangeEvent(notification:)),
//                                       name: AVAudioSession.routeChangeNotification, object: nil)
//        notificationCenter.addObserver(self, selector: #selector(OTDefaultAudioDevice.appDidBecomeActive(notification:)),
...
//        areListenerBlocksSetup = true
    ...
    fileprivate func setupAudioSession() {
        ...
        do {
            try session.setPreferredSampleRate(Double(OTDefaultAudioDevice.kSampleRate))
...
//                AVAudioSession.CategoryOptions.defaultToSpeaker.rawValue
            try session.setCategory(AVAudioSession.Category.playAndRecord, mode: AVAudioSession.Mode.voiceChat, options: AVAudioSession.CategoryOptions(rawValue: audioOptions))
            setupListenerBlocks()
...
extension OTDefaultAudioDevice {
    ...
    func configureAudioSessionWithDesiredAudioRoute(desiredAudioRoute: String) {
        safetyQueue.async {
            let session = AVAudioSession.sharedInstance()
           
            if desiredAudioRoute == OTDefaultAudioDevice.kAudioDeviceBluetooth {
                self.setBluetoothAsPreferredInputDevice()
            }
            do {
                try session.setActive(false)
                if desiredAudioRoute == OTDefaultAudioDevice.kAudioDeviceSpeaker {
...
//                    do {
                    ...
                    try session.setCategory(AVAudioSession.Category.playAndRecord, mode: .voiceChat)
                    try session.setActive(true)
...
func otRenderCb(inRefCon:UnsafeMutableRawPointer,
...
{
    let audioDevice: OTDefaultAudioDevice = Unmanaged.fromOpaque(inRefCon).takeUnretainedValue()
    ...
    otUpdatePlayoutDelay(withAudioDevice: audioDevice)
    
    return noErr
}

func otRecordCb(inRefCon:UnsafeMutableRawPointer,
              ioActionFlags:UnsafeMutablePointer<AudioUnitRenderActionFlags>,
              inTimeStamp:UnsafePointer<AudioTimeStamp>,
              inBusNumber:UInt32,
              inNumberFrames:UInt32,
              ioData:UnsafeMutablePointer<AudioBufferList>?) -> OSStatus
{
    ...
    otUpdateRecordingDelay(withAudioDevice: audioDevice)
    
    return noErr
}

func otUpdatePlayoutDelay(withAudioDevice audioDevice: OTDefaultAudioDevice) {
    ...
    if audioDevice.playoutDelayMeasurementCounter >= 100 {
        ...
        audioDevice.playoutDelay += UInt32(interval * OTDefaultAudioDevice.kToMicroSecond)
        // HW buffer duration
        let ioInterval = session.ioBufferDuration
        audioDevice.playoutDelay += UInt32(ioInterval * OTDefaultAudioDevice.kToMicroSecond)
        audioDevice.playoutDelay += UInt32(audioDevice.playoutAudioUnitPropertyLatency * OTDefaultAudioDevice.kToMicroSecond)
        // To ms
...
func otUpdateRecordingDelay(withAudioDevice audioDevice: OTDefaultAudioDevice) {
    ...
    if audioDevice.recordingDelayMeasurementCounter >= 100 {
        ...
        audioDevice.recordingDelay += UInt32(interval * OTDefaultAudioDevice.kToMicroSecond)
        let ioInterval = session.ioBufferDuration
        
        audioDevice.recordingDelay += UInt32(ioInterval * OTDefaultAudioDevice.kToMicroSecond)
        audioDevice.recordingDelay += UInt32(audioDevice.recordingAudioUnitPropertyLatency * OTDefaultAudioDevice.kToMicroSecond)
